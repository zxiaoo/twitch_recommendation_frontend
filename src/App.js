...
class App extends React.Component {
  ...
  componentDidMount = () => {
    // stay login when refreshing the web:
    // send a request to backend to see if the session cookie is still valid
    // check the response status code
    // if 200 setState({loggedin: true}) else setState({loggedin: false})
    getTopGames()
      .then((data) => {
        this.setState({
          topGames: data
